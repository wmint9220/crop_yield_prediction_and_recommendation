...
def show_trend():
    ...
    with tab1:
        ...
        with st.expander("📖 **What do the Soil Nutrient (NPK) levels mean?**"):
            ...
            The gauge shows how close the crop's requirement is to the scale maximum.
            ...
            |--------|-------------|------------------------------|
            | 🔵 Low | < 45 | The crop needs very little Nitrogen. Over-fertilizing can burn the plant or cause excessive leaf growth at the expense of fruit/grain. Use minimal or no N-fertilizer. |
            | 🟢 Moderate | 45 – 90 | A balanced Nitrogen need. Standard fertilization schedules apply. Monitor leaf color; yellowing (chlorosis) signals deficiency. |
            | 🟠 High | > 90 | The crop is a heavy feeder. You will need multiple N-fertilizer applications (split dosing) throughout the season to sustain growth. |
            ...
            | Status | Value Range | What it means for your field |
            |--------|-------------|------------------------------|
            | 🔵 Low | < 45 | Minimal phosphorus needed. Common in root/tuber crops. Excess P can lock out Zinc and Iron in the soil. |
            | 🟢 Moderate | 45 – 90 | Standard P requirements. Apply basal phosphate fertilizer before sowing/planting. |
            | 🟠 High | > 90 | High demand, often seen in fruiting or flowering crops. Incorporate bone meal, superphosphate, or DAP at planting time. |
        ...
        with st.expander("📖 **What do the Climate & Soil Condition levels mean?**"):
            ...
            #### 🌡️ Temperature — *Metabolic Activity Rate*
            Temperature drives the rate of germination, photosynthesis, and nutrient uptake.

            | Status | Value Range | What it means for your field |
            |--------|-------------|------------------------------|
            | ❄️ Cool | < 20°C | The crop prefers cool conditions. Warm-season crops will fail or produce poorly. Ideal for highland or temperate-zone cultivation. |
            | 🌡️ Moderate | 20°C – 30°C | The optimal range for the majority of tropical and subtropical crops. Most fertilizers and pesticides work at their rated effectiveness. |
            | 🔥 Warm | > 30°C | The crop thrives in heat. Cool-season crops planted here will bolt, wilt, or die. Ensure adequate water availability to offset high evapotranspiration. |

            ---

            #### 💧 Humidity — *Atmospheric Moisture*
            Humidity affects how much water a plant loses through its leaves (transpiration) and disease pressure.

            | Status | Value Range | What it means for your field |
            |--------|-------------|------------------------------|
            | 🏜️ Dry | < 40% | Low atmospheric moisture. The crop is drought-adapted. High humidity crops planted here will suffer from water stress and reduced yield. Irrigation may be necessary. |
            | 💧 Moderate | 40% – 70% | A comfortable moisture level for most crops. Disease risk is manageable with standard fungicide/pest schedules. |
            | 💦 Humid | > 70% | High moisture environment. This crop is adapted to humid tropics. Fungal diseases (blight, mildew) are a major risk — monitor closely and use preventive fungicides. |

            ---

            #### 🌧️ Rainfall — *Total Water Input*
            Rainfall determines how much supplemental irrigation (if any) is needed throughout the growing season.

            | Status | Value Range | What it means for your field |
            |--------|-------------|------------------------------|
            | 🌵 Low | < 100 mm | The crop is drought-tolerant or grows in a short season. Regions with low annual rainfall can support this crop with minimal or no irrigation. |
            | 🌧️ Moderate | 100 mm – 200 mm | Standard rainfall requirement. Suitable for most semi-arid to sub-humid regions. Supplemental irrigation during dry spells may improve yield. |
            | ⛈️ High | > 200 mm | The crop demands significant water. Ensure good drainage to prevent waterlogging, which can cause root rot. In low-rainfall areas, this crop requires heavy irrigation infrastructure. |

            ---
            💡 **Pro-Tip:** pH, Temperature, Humidity, and Rainfall together define your crop's **ecological niche**. If your local conditions differ significantly from these values, consider soil amendments, irrigation, or shade structures — or choose a better-suited crop using the Crop Comparison tab.
            """)
        # ── END NEW expander ─────────────────────────────────────────────────
    ...
    with tab2:
        ...
        # ── NEW: Independent Mean/Median selector for Tab 2 ─────────────────
        st.markdown("### 📊 Statistical Measure Selection")
        ...
        # Recalculate the selected crop's values using Tab 2's own selector
        ...
        if compare_crops:
            # Build comparison data using Tab 2's own calculated_values_tab2
            comparison_data = {"Crop": [selected_crop] + compare_crops}
            ...
            for crop in compare_crops:
               # Calculate similarity score using Tab 2's selector
               if central_tendency_tab2 == "Mean":
                   crop_data = df[df["label"] == crop][features_row1 + features_row2].mean()
               else:  # Median
                   crop_data = df[df["label"] == crop][features_row1 + features_row2].median()
